...
class ImportDataController extends Controller
{
        ...
        try{
            ...
            foreach($vendorArr as $vendor){
                ...
                $user = User::where('vendor_code', '=',  $vendor->CardCode)->where('user_role', '=',  3)->first();
                    ...
                    $errors[] = array("BPCode" => "The line # ".$vendor->CardCode." and #Name ".$vendor->CardName.", List Num ".$vendor->ListNum.". Sales Person Code".$vendor->SlpCode);
                    $skipRow++;
                }
            }   
            
            $responseData = [
                'message'=>'Skip records: '.$skipRow.', Successfully inserted: '.$icout.', Successfully Updated: '.$ucout.' out of total '.$totalCount,
                'error'=>$errors,
    ...
    public function productImport()
    {
        ...
        /*   $json =  file_get_contents($url);
            ...
            if(!empty($errors)){
                $message = 'Skip records '.$skipRow.' out of total '.$totalCount.' successfully inserted '.$icout;
                // return response()->json(['message'=>
                // 'Skip records '.$skipRow.' out of total '.$totalCount.' successfully inserted '.$icout,
                // 'error'=>$errors],200); 
            }else{
    ...
    public function productPriceImport()
    {
        ...
        try {
            ...
            $responseData = [
                'message'=>'Total '. $count.' records inserted '. $insertcount,
                'error'=>'',
                'function' => 'productPriceImport'            
            ];
            $send = User::sendNotificationForCron($responseData);
            return response()->json($responseData,200); 
